...
public class mainUtilisateur {
	...
	public static void main(String[] args) throws IOException {
		...
		//##################################################################################
		//## DEBUT DE LA PHASE 1 (VOIR CLASSES MAIN ANNUAIRE PUIS ENVOI FICHIER ANNUAIRE) ##
		//##################################################################################
		...
		Serv.sendObject(portstring + " " + addr, conAnn, Serv.getOutputS());

		//Saisie du répertoire qui contient les fichiers du serveur
		System.out.println("Veuillez entrer le nom du répertoire du serveur :");
		...
		while (true) {
			...
			//########################################################################
			//## A MODIFIER SI VOUS SOUHAITEZ CHANGER LE CHEMIN DE VOS UTILISATEURS ##
			//########################################################################
			String defaultPath = "D:\\Bureau\\" + finPath;
			//########################################################################
			
			//Modification du chemin en ajoutant le dernier morceau
			Serv.setpath(defaultPath);
			File rep = new File(Serv.getPath());
			System.out.println(Serv.getPath());
		...
		//#############################################################################
		//## DEBUT DE LA PHASE 2 (VOIR CLASSES MAIN ANNUAIRE ET RECUP LIST FICHIERS) ##
		//#############################################################################
		...
		for (int j=0; j<count; j++) {
			...
			while (it.hasNext()) {
				...
				int taill = tabInd.get(mentry.getKey());
				...
				Obj_fil obj = new Obj_fil(downable, splIPort[0], taill, splIPort[1]);
				lisObj.add(obj);
				
				//Affichage
				System.out.print("Serveur : "+ obj.getPort() + " & Blocs : ");
				System.out.println("[" + 1 + " => " + taill +"]");
		...
		//####################################
		//## VOIR CLASSE LANCEMENT SERVEUR  ##
		//####################################
		
		LancementServeur s = new LancementServeur(Serv);
		...
		while (true) {
			...
			while (Guilhem == 0) {
				System.out.println("1 pour Télécharger un fichier");
				System.out.println("2 pour Quitter");
				String state = Serv.saisieClavier();
			...
			for (int i=0; i<tabport.size(); i++) {
				...
				Client codl = new Client(Serv.getPath(), tabport.get(i), filedown, sizeMin, sizeMax, i, ip);
				clithre.add(codl);
				//Démarrage du Thread
